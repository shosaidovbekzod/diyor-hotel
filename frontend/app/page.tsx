    ...
    <div className="pb-16">
        ...
        <div className="shell relative grid min-h-[86vh] gap-10 py-16 lg:editorial-grid lg:items-end lg:py-24">
          <div className="max-w-4xl self-end pb-8">
            ...
            <div className="mt-10 flex flex-wrap gap-4">
              <Link
                href="/rooms"
                className="rounded-full bg-white px-7 py-3 text-sm uppercase tracking-[0.22em] text-ink transition hover:bg-[#f1e8da]"
              >
                {copy.explore}
              </Link>
              <a
                href={summary.telegram_url}
                className="rounded-full border border-white/25 px-7 py-3 text-sm uppercase tracking-[0.22em] text-white transition hover:bg-white/10"
              >
                {copy.telegram}
              </a>
            </div>
          ...
          <div className="glass-panel self-end rounded-[34px] border border-white/15 p-7 text-ink shadow-luxe">
            ...
            <form action="/rooms" className="mt-8 space-y-4">
              ...
              <input
                ...
                className="w-full border-b border-[#d6cab9] bg-transparent px-0 py-3 text-sm outline-none"
              />
              <button className="mt-4 w-full rounded-full bg-ink px-6 py-4 text-sm uppercase tracking-[0.22em] text-white transition hover:bg-[#2a251f]">
                {copy.search}
              </button>
            </form>
          </div>
      ...
      <section className="shell mt-20 grid gap-12 lg:editorial-grid lg:items-start">
        <div>
          ...
          <h2 className="mt-4 max-w-3xl font-display text-5xl leading-tight text-ink">
            Spacious stays with a quiet, residential sense of luxury.
          </h2>
          <p className="mt-6 max-w-2xl text-base leading-8 text-ink/72">
            Every room is designed with soft light, restrained materials, and the kind of calm rhythm
            expected from a premium city hotel. The result is a stay that feels private, composed, and
        ...
        <div className="space-y-5 border-t border-[#d8cfc2] pt-5 text-sm text-ink/72">
          ...
          <Link
            ...
            className="inline-flex rounded-full border border-[#cbb99a] px-6 py-3 text-xs uppercase tracking-[0.24em] text-ink transition hover:bg-[#ebe1d2]"
          >
            {copy.reserveOffer}
          </Link>
        </div>
      ...
      {leadRoom ? (
        <section className="shell mt-16 grid gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
          <div className="relative min-h-[540px] overflow-hidden rounded-[38px]">
            <Image src={leadRoom.image_url} alt={leadRoom.title} fill className="object-cover" />
          </div>
          <div className="card rounded-[38px] p-8 md:p-10">
            ...
            <div className="mt-10 flex items-end justify-between gap-6 border-t border-[#e6ddd2] pt-6">
              ...
              <Link
                ...
                className="rounded-full bg-ink px-7 py-3 text-xs uppercase tracking-[0.24em] text-white transition hover:bg-[#2c2721]"
              >
                {copy.viewAll}
              </Link>
            </div>
      ...
      <section className="shell mt-20 grid gap-6 lg:grid-cols-2">
        {supportingRooms.map((room) => (
          <Link key={room.id} href={`/rooms/${room.slug}`} className="card overflow-hidden rounded-[34px]">
      ...
      <section className="shell mt-24 grid gap-12 lg:editorial-grid lg:items-start">
        <div>
          ...
          <h2 className="mt-4 max-w-3xl font-display text-5xl leading-tight text-ink">
            Wellness rituals and all-day hospitality, composed with quiet confidence.
          </h2>
      ...
      <section className="shell mt-24 rounded-[38px] bg-[#1c1814] px-8 py-12 text-white md:px-12">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {summary.testimonials.map((review) => (
              <div key={review.id} className="border-l border-white/15 pl-5">
                <div className="font-display text-2xl text-champagne">{review.rating.toFixed(1)}</div>
